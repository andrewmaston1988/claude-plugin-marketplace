...
| `remote.createChannels` | `false` | `true` → `/slack-remote` creates `#ln-<peer-id-short>` (requires `channels:write` + `channels:manage` on the bot token). `false` → DM-seize (no new scopes). |
...
The one-time MCP registration is **user-scoped** (`claude mcp add --scope user slack-bridge-remote -- node <path>/bin/claude-slack.mjs remote-mcp`), not plugin-declared — plugin-declared MCP servers do not render `notifications/claude/channel` as of 2026-07-16, so the live session would never receive inbound Slack messages. The wizard runs the registration; to do it by hand see README → *Remote control → Enabling*.
