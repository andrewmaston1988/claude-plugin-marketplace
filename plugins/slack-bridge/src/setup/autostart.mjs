 ...
 * @param {{ nodePath: string, bridgeEntry: string, configDir: string, logDir: string }} tokens
...
/**
 * Install the autostart entry for the given platform.
 *
 * @param {"win32"|"darwin"|"linux"} platform
 * @param {string} renderedContent - output of renderTemplate()
 * @param {{ log: object }} opts
 */
export async function installAutostart(platform, renderedContent, { log } = {}) {
  if (platform === "win32") {
    const tmp = join(homedir(), "AppData", "Local", "Temp", "claude-slack-task.xml");
...
/**
 * Uninstall the autostart entry for the given platform.
 *
 * @param {"win32"|"darwin"|"linux"} platform
 */
...
/**
 * Verify the autostart entry is registered.
 *
 * @param {"win32"|"darwin"|"linux"} platform
