...
The UserPromptSubmit hook owns a **self-correcting** cadence (jitter-aware, target ~85% of the cache TTL) and logs each tick to `~/.claude/.amag-checkpoint-keepalive.jsonl`. Verify with `/keepalive-status`.
