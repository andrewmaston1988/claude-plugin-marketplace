...
2. **SessionStart hooks** — two, split by source. On a fresh start (`startup`/`clear`) with an existing STATE.md, offers to resume. On `compact`, points the session that just compacted at its own STATE file — naming the path and what is actually in it (a fresh skeleton, an existing checkpoint, or nothing), since PreCompact skips the write when a real checkpoint is already there. The harness event is what scopes this to the right session. Opt out (both): `checkpoint.sessionStartResume: false`.
