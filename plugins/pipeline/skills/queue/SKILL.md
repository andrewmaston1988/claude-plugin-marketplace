...
**Cross-project prerequisites.** A `*Prerequisites:*` token may name another project as `project:feature` (e.g. `example-service:PROJ-102-example-research`). These are always **soft** — the row holds until that other-project row reaches `done`. They are never `waits_on` (`!project:feature` and a cross-project `--waits-on` are rejected, because the ancestor check only works within one git repo). `queue-plan` validates that the named project is registered.
