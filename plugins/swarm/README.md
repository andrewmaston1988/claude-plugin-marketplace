...
A read-only LAN web page over `~/.swarm/runs`: every project's runs, the live roster of a run drawn as its own topology (a git-graph rail from the manifest's `after` edges and the `expand` / `expand-manifest` events — each line owns its lane until it lands, so no line ever runs behind a dot that is not its own; fan-in into dependents, digest at the foot), a leaf's place in the graph plus its timing, tokens, activity and output, and a finished run's digest or report rendered through the same converter as `report`. A wave starts open, collapsing to one row with a dot strip only once you tap it closed. A `forEach` is drawn trunk-and-branch: its row is a label on its upstream's line (no dot of its own), its clones follow directly beneath it clone by clone — for a manifest `forEach`, each clone's sessions (`walk → extend → verify`) in a straight line of their own — each clone steps one lane off the trunk and merges back into it, and the trunk carries every clone on to the row that waits on the `forEach`. A line is coloured by the rollup of everything it carries, so a trunk turns purple the moment a merged clone is running and green only once every clone above it is done. A nested manifest is its own node, tapping through to a screen scoped to just that node and its children (`#/run/<project>/<name>/node/<id>`), drawn through the same graph renderer as the run screen. A project's finished-run stack starts collapsed and opens on tap, so a busy estate's live band stays in reach; a `Show all N` row at the foot of an expanded stack fetches that project's remaining finished runs uncapped (one opt-in round trip, per project, dropped again on collapse).
