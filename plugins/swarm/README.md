...
A kept worktree is the salvage, not litter — it survives on purpose so the session can merge from it. `swarm prune <resultsDir>` destroys one run's kept worktrees and their branches once that work has landed: it refuses a live run, prints every tree's path, branch, size and landing state (`merged` / `unlanded (<n> commits ahead)` / `dirty`), then removes them and closes with `freed <N> GB across <k> worktrees`. Run `--dry-run` first — same table, nothing touched. Nothing prunes on its own: the workflow-discipline closing phase pauses with the project's finished runs that still hold kept worktrees and their dry-run tables, and the session judges which of them this PR landed before typing `prune <resultsDir>` per run.
