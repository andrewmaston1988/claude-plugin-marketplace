...
// ── Hand-expanded list that should be a forEach ──────────────────────────────
// S5, three times over: a session runs the discovery grep itself and bakes the
// result into N near-identical static leaves instead of a `find` task + forEach.
// validate must reject it — prose lost this to the batching arithmetic twice.

test("validate rejects sibling leaves that differ only by a path — name forEach", () => {
  const dir = tmp();
  const files = ["dispatch.mjs", "scheduler.mjs", "worktree.mjs", "discovery.mjs"];
  const p = writeManifest(dir, {
    tasks: files.map((f, i) => ({
      id: `review-${i}`,
      model: "haiku",
      prompt: `Review plugins/swarm/src/${f} for unescaped child_process args. Report file:line and a verbatim quote.`,
    })),
  });
  const errs = errorsOf(() => loadManifest(p, CFG, dir));
  ok(errs.some((e) => /forEach/.test(e) && /hand-expanded|per[- ]?item|same shape/i.test(e)),
     "must flag the hand-expanded list and name forEach: " + JSON.stringify(errs));
});

test("validate accepts sibling leaves that genuinely differ (not a path swap)", () => {
  const dir = tmp();
  const p = writeManifest(dir, {
    tasks: [
      { id: "a", model: "haiku", prompt: "Summarise the README." },
      { id: "b", model: "haiku", prompt: "List every TODO in the tree." },
      { id: "c", model: "haiku", prompt: "Draft a migration guide." },
    ],
  });
  const plan = loadManifest(p, CFG, dir); // must not throw
  equal(plan.tasks.length, 3);
});

test("validate does not flag a real forEach as a hand-expanded list", () => {
  const dir = tmp();
  const p = writeManifest(dir, {
    tasks: [
      { id: "find", model: "haiku", prompt: "List every .mjs importing child_process. Return an array." ,
        returns: { type: "array", items: { type: "string" } } },
      { id: "review", model: "haiku", after: ["find"],
        forEach: { from: "find", maxItems: 30 },
        prompt: "Review {{item}} for unescaped child_process args." },
    ],
  });
  const plan = loadManifest(p, CFG, dir); // must not throw
  equal(plan.tasks.length, 2);
...
test("two path-only siblings are below the threshold — not flagged", () => {
  const dir = tmp();
  const p = writeManifest(dir, {
    tasks: [
      { id: "a", model: "haiku", prompt: "Review src/dispatch.mjs for X." },
      { id: "b", model: "haiku", prompt: "Review src/scheduler.mjs for X." },
    ],
  });
  const plan = loadManifest(p, CFG, dir); // 2 is a pair, not a hand-expanded list
  equal(plan.tasks.length, 2);
});
