...
test("read-only digest reads the dispatching repo in place, like any other reader", () => {
  const t = buildDigestTask(plan({ repoToplevel: "C:/work" }));
  equal(t.cwd, "C:/work");
  equal(t.worktreeName, undefined, "the digest reads; it owns no tree");
  equal("scratchRedirect" in t, false);
});

test("report digest sits in scratch-__digest, not in the repo it reports on", () => {
  const t = buildDigestTask(plan({ repoToplevel: "C:/work", digest: { model: "haiku", report: true } }));
