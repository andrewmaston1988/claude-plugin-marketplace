...
Liveness: the engine paints a heartbeat file every `heartbeatSecs` (`~/.swarm/config.json`, default 15) — that same interval is the liveness window (3x), so a stale heartbeat is what tells `status`/`stop` an engine died without a summary. `node <engine> stop <resultsDir>` is the only sanctioned way to end a run early.
