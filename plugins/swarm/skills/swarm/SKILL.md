...
- **Do NOT kill anything** — no `Stop-Process`/`taskkill`, no killing the shell. There is no per-leaf kill. Ending a live engine is `swarm stop <resultsDir>` — a cooperative signal-and-wait, never a process kill — and it is the operator's call alone, not yours to invoke on a hunch.
...
Dispatch is **deny-by-default, for every provider including Claude**. `allowedRoots` in `~/.swarm/config.json` lists the directory roots where tasks may run; a task whose effective `cwd` is not under its provider's roots **fails validation**, and an empty list permits nothing. That is ONE top-level list, the default for every provider — `providers.<name>.allowedRoots` can only NARROW it, never widen it, because the two are intersected. For a non-Claude provider the reason is the data agreement — the employer's covers Anthropic only, so code outside those roots must never reach another provider. For Claude it is containment: swarm runs nothing outside its configured roots. With no list configured at all, nothing dispatches until one is written, which is what `swarm setup` writes. When a manifest is rejected on governance grounds, move the work under an allowed root or configure the root — never work around the gate.
...
5. **Run**: dispatch from the repo the work belongs to and pass the manifest by absolute path (`cd <repo>; swarm run "<abs manifest path>"`) — the run is filed under that repo's git toplevel, and a cwd outside any repo, or a repo outside `provider.allowedRoots`, is refused. `swarm run <manifest.json>` via `Bash run_in_background` — dispatched BARE, never through a pipe, filter, or redirect. Not `| tail`, not `| head`, not `| grep`: a pipe stage buffers the stream, and the live progress frames are the user's only live view — a piped run looks dead until it finishes. "Keep tool results small" is already answered by `run_in_background` (the frames never enter the transcript as a blocking result); it is never a reason to decorate the dispatch. The completion notification is the "run finished" signal; if the host reaped that wrapper (low memory) and no notice ever arrives, dispatch `swarm wait <resultsDir>` with `run_in_background` — it blocks until the run settles, prints the final roster, and gives you that notice.
   ...
   **Status asks**: when the user asks how the swarm is doing ("/swarm status", "how far along…"), run `swarm status <resultsDir>` once and render the roster as a **markdown table** (state | leaf | model | time | tokens, glyphs kept — the TUI renders markdown; a table beats raw monospace).
...
6. **Read `digest.md` ONLY**, then drill into `results/<id>.json` selectively — the digest's drill-down section says which raw results merit a full read. Never read all raw output. For a targeted follow-up on one leaf's finding (a citation to verify, a claim to expand), prefer `swarm ask <resultsDir> <leaf-id> "<question>"` over re-running or reading raw output: it resumes the leaf's own session — context intact, one turn, answer on stdout. An ask runs through the run's own engine, so it shows on `status`/dashboard as that leaf running again and cannot overlap a still-live run (or vice versa).
6b. **Grade the run's leaves when `grading.enabled` is on — the closing block then asks you to, by name, and the session Stop hook lists any ungraded runs this session dispatched as a backstop.** Off (the shipped default) the closing block is silent, the digest footer stays empty, and this step is skipped; `grade` and `perf` still answer when called by hand. Its last line names the count and hands you the exact command; that line is printed by the engine, so it appears whether or not you were carrying this step in mind. You authored the manifest, so you are the only party that knows what each leaf was asked for — the digest holds the outputs but not the intent, and on many runs there is no digest leaf at all. Run `swarm grade --init <resultsDir>`; it writes `grades.json` with one row per model leaf — Claude tiers included, so the ranking compares families (operator, 2026-08-31); agentless nodes carry no model and get no row. Fill in `session`, and per row `domain`, `outcome`, a `note`, and the grades 1-10:
...
8. **For a human-facing report, RENDER it — never hand-author one.** When a run wrote `report.md` (step 5b), project it to a self-contained, theme-aware `report.html` with `swarm report <resultsDir>`. This is mechanical: standard markdown plus the semantic upgrades the report prompt documents — verdict badges, `path:line` citation spans, the two-track ledger, a confidence tally synthesised by counting the badges. It writes `report.html` beside `report.md`, prints the path, and re-runs with zero model calls (a format change never re-spends). Offer that path; do not build an Artifact by hand from `summary.json`.
9. **`prune <resultsDir>`** destroys a run's kept worktrees and their branches, never its results — run `--dry-run` first. The operator's call, never the leaf's and never automatic; the workflow-discipline closing phase reminds you which runs still hold worktrees.
...
Liveness: the engine paints a heartbeat file every `heartbeatSecs` (`~/.swarm/config.json`, default 15) — that same interval is the liveness window (3x), so a stale heartbeat is what tells `status`/`stop` an engine died without a summary. `swarm stop <resultsDir>` is the only sanctioned way to end a run early.
