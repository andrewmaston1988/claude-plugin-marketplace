...
// not, 2 the engine died before it settled anything, 3 --timeout ran out first.
...
export async function runWaitCommand(dir, { quietWarnSecs = 60, timeoutSecs = null, out, err, ...opts } = {}) {
  const r = await waitForRun(dir, { quietWarnMs: quietWarnSecs * 1000, timeoutMs: timeoutSecs && timeoutSecs * 1000, ...opts });
...
export async function waitForRun(dir, {
  ...
  for (;;) {
    ...
    if (timeoutMs != null && at - startMs >= timeoutMs) {
      return { code: EXIT_TIMEOUT, run, message: `swarm: still running after ${Math.round((at - startMs) / 1000)}s — the run is untouched; wait again or check status.` };
    }
