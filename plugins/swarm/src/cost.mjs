...
// developers.openai.com/api/docs/pricing, the published per-model table.
...
// Standard-tier columns only: the batch, flex and fast-mode columns are fixed
// multiples of these and swarm dispatches none of them. Astra's figures are its
// sub-272k tier — it is the one row with breakpoint pricing ($20/$75 above).
// gpt-5.5-pro and gpt-5.5-cyber are API-only and never reach the Codex CLI, so
// they are absent rather than priced.
...
// platform.claude.com/docs/en/about-claude/pricing, the published model table,
// read 2026-09-23. Every row is now its own published line, so nothing is
// carried `via` a sibling — a family does NOT share one price: sonnet-4-6 bills
// $3/$15 against sonnet-5's $2/$10, and opus-5-5 undercuts opus-5 at $4/$20.
// Cache reads are 0.1x base input, except opus-5-5 (0.05x) and fable-5-1
// (0.025x); they are stored rather than derived so a published change is edited
// where it was read.
export const CLAUDE_RATE_CARD = {
  ...
  staleAfter: defaultRateCardStaleAfter("2026-09-23"),
  prices: {
