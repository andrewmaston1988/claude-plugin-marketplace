...
| Tool | Purpose |
|---|---|
| `list_peers` | Discover other sessions plus your own row, marked (`scope`: `machine` / `directory` / `repo`) |
...
[This agent (7f2a9c31)]
  ID: 7f2a9c31
  PID: 41022
  CWD: C:/code/primordial
  Checkout: C:/code/primordial
  Summary: primordial TOWER — landing order set, waiting on porter B's mutation sweep.
  Summary set: 2026-08-23T17:44:02.118Z
  Last seen: 2026-08-23T17:52:20.004Z
...
**Your own row leads the list, marked `[This agent (<id>)]`.** It is the only view of what you have actually published — whether the summary landed, and how old it is. The count line reports *other* peers, so it never counts you; when there are none, your row still prints above `No other Claude Code instances found`. A registered id the broker no longer holds prints as `[This agent (<id>)] — not in the broker's registry`, which is the failure the row exists to surface: without it, a lost row looks exactly like a healthy one.
